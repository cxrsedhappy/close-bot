...
from data.db_session import create_session, Player, Role
...
class EconomyCog(commands.Cog):
    ...
    async def reward(self, interaction: discord.Interaction):
        ...
        embed.set_author(name=f'Временная награда {interaction.user.name}',
    ...
    async def bonus_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        if isinstance(error, app_commands.CommandOnCooldown):
            ...
            embed = Embed(colour=2829617)
            embed.set_author(name=f'Временная награда {interaction.user.name}',
                             icon_url=interaction.user.display_avatar.url)
            embed.description = f'Вы уже забирали награду, следующий раз можно получить через {h}:{m}:{s}'
            await interaction.response.send_message(embed=embed)
